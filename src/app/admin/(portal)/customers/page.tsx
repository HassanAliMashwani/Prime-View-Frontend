        ...
        <form onSubmit={handleSubmitPathA} className="space-y-6">
          ...
          <div className="bg-white rounded-2xl border border-slate-200/80 p-6 shadow-xs space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              ...
              {/* 2. Applicant CNIC Copy */}
              <div className="rounded-2xl border border-slate-200 p-4 bg-slate-50/50 flex flex-col justify-between space-y-3">
                <div>
                  <div className="flex items-center justify-between mb-1">
                    <label className="text-xs font-bold text-slate-800">Applicant CNIC Copy</label>
                    {docFiles.cnic ? (
                      <span className="inline-flex items-center gap-1 text-[10px] font-bold text-emerald-700 bg-emerald-100 px-2 py-0.5 rounded-md">
                        <Check className="w-3 h-3" /> Attached
                      </span>
                  ...
                  <p className="text-[11px] text-slate-500 mb-3">Verified front &amp; back copy</p>
                </div>

                {docFiles.cnic ? (
                  <div className="relative group rounded-xl border border-emerald-300 bg-white p-2.5 flex items-center gap-3">
                    {docFiles.cnic.isPdf ? (
                      <div className="w-14 h-14 rounded-lg bg-rose-50 border border-rose-200 text-rose-600 flex items-center justify-center shrink-0">
                        <FileText className="w-6 h-6" />
                      </div>
                    ) : (
                      <img
                        src={docFiles.cnic.dataUrl}
                        alt="Applicant CNIC Preview"
                        className="w-14 h-14 object-cover rounded-lg border border-slate-200 shrink-0"
                      />
                    )}
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-bold text-slate-800 truncate">{docFiles.cnic.name}</p>
                      <p className="text-[10px] text-slate-500">{docFiles.cnic.sizeKb} KB • {docFiles.cnic.isPdf ? 'PDF document' : 'Image document'}</p>
                    </div>
                    <button
                      type="button"
                  ...
                  <label className="relative border-2 border-dashed border-slate-300 hover:border-emerald-600 rounded-xl p-5 flex flex-col items-center justify-center gap-2 bg-white hover:bg-emerald-50/40 cursor-pointer transition group">
                    ...
                    <div className="text-center">
                      ...
                      <span className="text-[10px] text-slate-400">PDF, JPG, or PNG (front &amp; back)</span>
                    </div>
                  </label>
                )}
              </div>

              {/* 3. Next of Kin CNIC Copy */}
              <div className="rounded-2xl border border-slate-200 p-4 bg-slate-50/50 flex flex-col justify-between space-y-3">
                <div>
                  <div className="flex items-center justify-between mb-1">
                    <label className="text-xs font-bold text-slate-800">Next of Kin CNIC Copy</label>
                    {docFiles.nokCnic ? (
                      <span className="inline-flex items-center gap-1 text-[10px] font-bold text-emerald-700 bg-emerald-100 px-2 py-0.5 rounded-md">
                        <Check className="w-3 h-3" /> Attached
                      </span>
                  ...
                  <p className="text-[11px] text-slate-500 mb-3">Verified nominee CNIC</p>
                </div>

                {docFiles.nokCnic ? (
                  <div className="relative group rounded-xl border border-emerald-300 bg-white p-2.5 flex items-center gap-3">
                    {docFiles.nokCnic.isPdf ? (
                      <div className="w-14 h-14 rounded-lg bg-rose-50 border border-rose-200 text-rose-600 flex items-center justify-center shrink-0">
                        <FileText className="w-6 h-6" />
                      </div>
                    ) : (
                      <img
                        src={docFiles.nokCnic.dataUrl}
                        alt="Next of Kin CNIC Preview"
                        className="w-14 h-14 object-cover rounded-lg border border-slate-200 shrink-0"
                      />
                    )}
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-bold text-slate-800 truncate">{docFiles.nokCnic.name}</p>
                      <p className="text-[10px] text-slate-500">{docFiles.nokCnic.sizeKb} KB • {docFiles.nokCnic.isPdf ? 'PDF document' : 'Image document'}</p>
                    </div>
                    <button
                      type="button"
                      onClick={() => handleRemoveFile('nokCnic')}
                      className="p-1.5 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors cursor-pointer"
                      title="Remove file"
                    >
                      <X className="w-4 h-4" />
                  ...
                  <label className="relative border-2 border-dashed border-slate-300 hover:border-emerald-600 rounded-xl p-5 flex flex-col items-center justify-center gap-2 bg-white hover:bg-emerald-50/40 cursor-pointer transition group">
                    <input
                      ...
                      accept="image/*,.pdf"
                      className="sr-only"
                      disabled={docUploading === 'nokCnic'}
                      onChange={(e) => {
                        const file = e.target.files?.[0];
                        if (file) handleFileUpload('nokCnic', file);
                      }}
                    />
                    <div className="w-10 h-10 rounded-full bg-slate-100 group-hover:bg-emerald-100 text-slate-500 group-hover:text-emerald-700 flex items-center justify-center transition-colors">
                      {docUploading === 'nokCnic' ? (
                        <div className="w-4 h-4 border-2 border-emerald-600 border-t-transparent rounded-full animate-spin" />
                      ) : (
