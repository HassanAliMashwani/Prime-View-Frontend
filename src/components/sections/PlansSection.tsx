      ...
      <div className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-12 relative z-10">
                ...
                <button
                  ...
                  className={`px-5 py-2 rounded-xl text-xs font-sans font-semibold tracking-wide transition-colors duration-150 ${
                    isActive
                      ...
                      : "bg-pure-white text-charcoal/80 border border-card-border hover:border-secondary-border hover:bg-secondary-hover-bg"
                  }`}
                >
                  {filter}
                </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
          {filteredPlans.map((plan) => (
            <div
              key={plan.id}
              className="bg-pure-white rounded-2xl border border-card-border p-7 flex flex-col justify-between hover:border-secondary-border hover:shadow-xs transition-colors duration-150"
            >
              <div>
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span className="font-mono text-[11px] text-muted-gray-text">Verified Plan</span>
                </div>
                ...
                <div className="flex items-center text-xs text-muted-gray-text mt-1 mb-6 gap-1.5">
                  <Calendar className="w-4 h-4 text-accent-green shrink-0" />
                  <span className="font-medium">{plan.installmentPeriod}</span>
                </div>

                <div className="border-t border-card-border/60 pt-5 space-y-3">
                  <span className="text-[11px] font-semibold uppercase tracking-wider text-muted-gray-text block">
                    Included Highlights
                  </span>
                  {plan.features.map((feat, idx) => (
                    <div key={idx} className="flex items-start text-xs sm:text-sm text-charcoal/85 gap-2.5">
                      <div className="w-4 h-4 rounded bg-green-tint-bg text-accent-green flex items-center justify-center shrink-0 mt-0.5">
              ...
              <div className="mt-8 pt-5 border-t border-card-border/60">
                <Link
                  href="/contact"
